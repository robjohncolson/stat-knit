...
Separately, a randomized crossover trial published in
\emph{Scientific Reports} (Kestin et al., 2025, $N = 194$ eligible
undergraduates) compared a scaffolded AI tutor to research-based
active-learning classroom instruction on identical physics content.
Reported result: the AI-tutor condition had a post-test median of
4.5 compared with 3.5 in active learning; a Mann--Whitney rank-sum
test gave $z = -5.6$ and $p < 10^{-8}$. The authors report a
regression-based effect size around $d = 0.63$, with ceiling-adjusted
quantile-regression estimates from about 0.73 to 1.3 SD.
...
\section{(b) Exact binomial test on the local outcome \textnormal{\small(Units 4 \& 6)}}

Test $H_0: p = 0.237$ vs. $H_a: p > 0.237$ where $p$ is the probability
a student in this class scores a 1. A one-proportion $z$-test is not
AP-appropriate here because the Large Counts condition fails:
\[
np_0 = 22(0.237) \approx 5.21 < 10.
\]
Instead, under $H_0$ let $X \sim \text{Binomial}(22, 0.237)$, where
$X$ is the number of students scoring a 1. Since all 22 students
scored a 1, the exact one-sided p-value is
\[
P(X \geq 22) = P(X = 22) = 0.237^{22} \approx 1.8 \times 10^{-14}.
\]
\textbf{Reject $H_0$.} The local 1-rate is astronomically higher than
the national benchmark. Then immediately explain why this rejection
does \emph{not} support the father's causal claim --- go to (c).

\begin{notebox}
This is a useful AP Statistics trap: the evidence is overwhelming,
but the normal approximation is still the wrong method. Students must
notice both facts.
\end{notebox}
...
\section{(d) Interpreting the published RCT \textnormal{\small(Units 3 \& 7)}}

The Kestin et al. study should not be rewritten as a simple two-sample
$t$-test. It was a randomized crossover design, and the published
comparison used a Mann--Whitney rank-sum test on post-test scores:
\[
\text{AI median} = 4.5, \qquad
\text{active-learning median} = 3.5, \qquad
z = -5.6, \qquad p < 10^{-8}.
\]
The authors also report a regression-based effect size around
$d = 0.63$, with ceiling-adjusted quantile-regression estimates from
about 0.73 to 1.3 SD.

Interpretation: if the AI tutor and active-learning conditions were
equally effective, a difference this extreme would be extraordinarily
unlikely by chance. The randomized crossover design supports a causal
claim for this setting: a carefully scaffolded AI tutor outperformed a
research-based active-learning class on these short physics lessons.

\begin{notebox}
The comparator was not weak lecture. It was active learning with peer
instruction and instructor feedback. That makes the result more
interesting, but it also means the claim should be stated precisely:
structured AI tutoring beat this active-learning implementation on
these tasks, not ``AI always beats teachers.''
\end{notebox}

\section{(e) Design conditions and the ``we don't have the data yet'' claim \textnormal{\small(Units 3 \& 6)}}

The claim ``we don't have the data'' is too broad. We do have strong
evidence that a \emph{scaffolded} AI tutor can improve short-term
learning in a carefully designed randomized setting. But the scope of
inference is limited by context: Harvard undergraduates, two physics
topics, first-encounter learning, expert-written prompts and solutions,
and short-term post-tests.

A statistically careful response is:
\begin{quote}
The RCT is real evidence against the claim that AI tutoring is merely
cheating or educationally empty. It is not evidence that every AI use
case works. The treatment that worked was structured, explanation-first
AI tutoring under a designed instructional system.
\end{quote}
...
\section{(f2) Guardrails: tutoring versus crutching \textnormal{\small(Unit 3)}}

Bastani et al. (2025, \emph{PNAS}) studied nearly 1,000 high-school
math students and found an important warning. During practice, GPT-4
access improved assisted performance. But when AI access was removed
on later exams, students in the unrestricted GPT condition performed
worse than control students. A scaffolded tutor condition, designed to
give hints rather than simply hand over answers, largely mitigated the
harm.

This distinction matters statistically because ``AI access'' is not
one treatment. At least two treatments must be separated:
\begin{itemize}[leftmargin=*]
  \item \textbf{Unrestricted answer engine:} can improve assisted
  practice performance while weakening independent mastery.
  \item \textbf{Scaffolded tutor with independent demonstration:}
  uses AI during learning but requires handwritten reasoning, verbal
  defense, and no-AI transfer tasks.
\end{itemize}

The relevant pedagogical question is therefore not ``AI or no AI?''
but \textbf{which design of AI use produces transferable skill?}
...
underserved. This is consistent with the broader tutoring hypothesis:
personalized, feedback-rich support should matter most for students
who were least served by group-paced instruction.
...
\begin{enumerate}[leftmargin=*]
  ...
  question (a scaffolded-tutor RCT --- yes; local ITT outcome under
  boycott --- no).
  \item States the \textbf{direction and magnitude} of the
  admissible evidence (AI median 4.5 vs. active-learning median 3.5,
  $z=-5.6$, $p < 10^{-8}$, regression-based $d \approx 0.63$ with
  ceiling-adjusted estimates about 0.73--1.3 SD).
  \item Identifies the \textbf{failure mode} of the local
  ``evidence'' (selection bias, confounding, near-total
  non-compliance).
  \item Concludes that the defensible empirical claim is:
  \emph{``Scaffolded AI tutoring, paired with independent
  demonstration, can improve skill acquisition; unrestricted
  answer-giving AI is a different treatment and can create a crutch
  effect.''} Arguments based only on the local all-1s outcome rely on
  inferential errors a Unit 3 / Unit 6 student is expected to identify.
\end{enumerate}
...
(b) & Exact binomial test; conditions for proportion inference \\
(c) & Estimand definition, ITT vs PP, interpretation under non-compliance \\
(d) & Interpreting a published randomized crossover study \\
(e) & Scope of inference, design conditions, evidentiary standards \\
(f) & Confounding, randomization as causal license \\
(f2) & Treatment definition: scaffolded tutoring versus answer engine \\
