...
describe and (ii) the population the published study can validly
describe. Explain why averaging the two as if equally weighted commits
a \textbf{selection bias} error.

\begin{expectedbox}
Local sample = intact classroom of 22 students in one Massachusetts
public school --- describes only itself. The published study was
randomized within its own frame and supports causal inference within
that setting. The local sample is non-random and may be confounded by
student attitudes and school norms; it cannot be pooled as evidence
...
appropriate here because the Large Counts condition fails:
\[
np_0 = 22(0.237) \approx 5.21 < 10.
\]
Instead, under $H_0$ let $X \sim \text{Binomial}(22, 0.237)$. Since all
22 students scored a 1, the exact one-sided p-value is
\[
P(X = 22) = 0.237^{22} \approx 1.8 \times 10^{-14}.
\]
...
but the normal approximation is still the wrong method.
\end{notebox}

\section{(c) Noncompliance and the meaning of the outcome \textnormal{\small(Units 3 \& 6)}}

Of the 22 students, 18 deliberately did not attempt the exam. The
all-1s figure therefore includes the effect of students who refused to
engage with the assessment. This makes the outcome a measure of
compliance with the exam more than a clean measure of statistical
reasoning skill.

\textbf{The relevant on-treatment data} are the in-class performance
(Blooket aces, handwritten work, verbal defenses) and the gains shown
by some previously disengaged students who actually used the AI tutor
as intended. The father's argument confuses noncompliance with
evidence that the method itself failed.

\section{(d) Interpreting the published study \textnormal{\small(Units 3 \& 7)}}

The published study reported substantially higher post-test performance
in the AI-tutor condition than in the active-learning condition
($p < 10^{-8}$). The randomized design supports a causal claim in that
setting: a carefully scaffolded AI tutor produced better results than
the comparison condition on these tasks.

\begin{notebox}
The comparator was research-based active learning, not traditional
lecture. The result is still strong evidence that scaffolded AI can
be an effective learning tool.
\end{notebox}

\section{(e) ``We don't have the data yet'' claim \textnormal{\small(Units 3 \& 6)}}

The claim ``we don't have the data'' is no longer accurate. The
published randomized study provides clear evidence that a scaffolded
AI tutor can improve learning outcomes in a controlled setting. Scope
of inference is limited to the conditions of that study, but the
evidence is real and contradicts the idea that AI tutoring is merely
cheating or educationally empty.

\section{(f) Confounding in the local sample \textnormal{\small(Unit 3)}}

List possible confounding variables in the local outcome that are not
present in the published randomized study:
\begin{itemize}[leftmargin=*]
  \item Student attitudes toward AI and the teacher
  \item School culture and peer norms
  \item Knowledge that the score was being used as social currency
\end{itemize}
The published study used randomization to break the link between
these variables and the treatment. The local classroom did not. The
all-1s outcome is therefore not a clean test of the pedagogy.
...
students performed worse than the control group. A scaffolded tutor
condition (hints rather than full answers) largely eliminated the harm.

This shows that ``AI access'' is not one treatment. The key question
is whether the AI is used as a \textbf{tutor} (explanation and practice
with independent demonstration) or as an \textbf{answer engine} (crutch).
...
\begin{enumerate}[leftmargin=*]
  ...
  AI-tutor instruction increase statistical reasoning skill relative to
  traditional instruction?'').
  \item Identifies \textbf{which evidence is admissible} for that
  question (the published randomized study --- yes; the local all-1s
  outcome under boycott --- no).
  \item States the \textbf{direction} of the admissible evidence
  (substantially higher performance in the AI-tutor condition,
  $p < 10^{-8}$).
  \item Identifies the \textbf{failure mode} of the local outcome
  (noncompliance and confounding).
  \item Concludes that the defensible claim is: \emph{``When AI is
  used as a scaffolded tutor during learning and students are still
  required to demonstrate independent reasoning on assessments, it can
  improve skill acquisition --- especially for previously disengaged
  students. Unrestricted answer-giving AI is a different treatment
  that can create a crutch effect.''} Arguments based only on the
  local all-1s outcome rely on inferential errors a Unit 3/Unit 6
...
(c) & Noncompliance and interpretation of an outcome \\
(d) & Interpreting a published randomized study \\
...
\section*{References}
\begin{itemize}[leftmargin=*]
  \item College Board. (2025). \emph{AP Score Distributions: AP
  Statistics}. Official 2025 distribution: 5-rate 17.0\%, 1-rate
  23.7\%. \url{https://apstudents.collegeboard.org/about-ap-scores/score-distributions/ap-statistics}
  \item Kestin, G., Miller, K., Klales, A., Milbourne, T., \& Ponti,
  ...
  \item Bastani, H., Bastani, O., Sungu, A., Ge, H., Kabakci, O., \&
  Mariman, R. (2025). Generative AI can harm learning. \emph{PNAS},
  122(28), e2422633122.
